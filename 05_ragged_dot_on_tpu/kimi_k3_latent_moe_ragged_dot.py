...
silently diffing against meaningless zero-input output. **Step 2 (not yet
done)**: real 896-expert top-k + local-id filtering (this padding scheme
carries over unchanged once that's real routing output, not a synthetic
draw), wired into the end-to-end forward pass/benchmark.
"""
...
def generate_local_shard_workload(
  ...
  """WP-Kimi step 2b, part 1 (review's P1-C item, per the user's two-step
  plan 2026-08-26): generate an isolated expert-kernel benchmark input with
  REALISTIC statistics for what one chip's `local_num_experts`-expert shard
  would see under a real `top_k`-of-`global_num_experts` global router,
  WITHOUT implementing that global routing/dispatch/filtering end-to-end
  (that's part 2: real 16-of-896 top-k -> filter to local ids, still not
  done here).
  ...
  **Fixed-total, tile-aligned padding via a single trailing bucket (rewritten
  2026-08-26, replacing a broken per-expert-capacity version from earlier the
  same day -- a review caught that the first version forced EVERY expert's
  `group_sizes` entry to the same constant `capacity`, which silently erased
  the real skew this whole benchmark exists to exercise: Mosaic's kernel
  specifically reacts to per-group sizes (WP3 notes: `make_group_metadata`
  skips near-empty groups, `num_tiles` scales with real counts), so a
  uniform group_sizes array quietly turns this back into the same
  too-regular workload `single_chip_kimi_k3_config` already had.):**
  ...
  expected total, rounded UP to `tile_size` -- the confirmed Mosaic tiling
  floor). This keeps the array SHAPE and VALUES identical across
  implementations without touching any real expert's group size.

  If the total raw draw exceeds `m_padded` (aggregate overflow -- unlikely
  at `capacity_factor=2.0`'s headroom, e.g. 2048 tokens/64 local
  experts/896 global -> expected total ~2340, m_padded rounds up to 4736,
  nearly 2x headroom), the excess is dropped from the tail of the
  expert-sorted order and a warning is printed with the drop count --
  documented, simple policy for a rare edge case, not a per-expert-fair
  scheme.
  ...
  Returns `(sorted_tokens, group_sizes, valid_mask, per_expert_counts)`:
    ...
    - `valid_mask`: `(m_padded,)` bool, True for rows holding real
      (non-padding) data. Use this to exclude padded rows before any
      correctness comparison (see check_shard_workload_correctness) --
      `int(valid_mask.sum())` is the true post-drop dispatched-token count.
    - `per_expert_counts`: `(local_num_experts,)`, same values as
      `group_sizes[:local_num_experts]` -- returned separately for
      reporting convenience (mean/min/max) without the caller needing to
      slice off the padding-bucket entry.

  Runs eagerly, not jit-compiled: m_padded and drop counts are
  data-dependent Python ints computed from the random draw, same
  "must run outside jit" constraint as the reference file's per-expert loop.
  """
...
def route_and_filter_to_local_shard(
  ...
  """WP-Kimi step 2b, part 2 (review's P1-C item): REAL global
  `config.top_k`-of-`config.num_experts` top-k routing (identical gate math
  to `latent_moe_forward_ragged_dot`'s step 1 -- float32 internally, cast
  back to compute dtype at the end), filtered down to the local shard
  ...
  (see kimi_k3_latent_moe_reference.py's module docstring); passing both in
  lets the caller reuse a single down-projection across shards/calls instead
  of recomputing it here.

  Not yet wired into an end-to-end forward pass or the tokamax benchmark
  (per the user's explicit two-step plan, 2026-08-26) -- this function and
  its combine-weight/token-index bookkeeping exist to be correctness- and
  padding/overflow-tested in isolation first (see
  check_route_and_filter_correctness), before any end-to-end wiring.
  ...
  padded_token_idx, padded_combine_weight)`:
    ...
    - `group_sizes`: `(local_num_experts + 1,)` -- real per-expert counts
      ...
      uniform -- same rationale as generate_local_shard_workload) plus one
      trailing padding-bucket size.
    - `valid_mask`: `(m_padded,)` bool, True for real (non-padding) rows.
    - `per_expert_counts`: `(local_num_experts,)`, same as
      `group_sizes[:local_num_experts]`, returned separately for reporting.
    - `padded_token_idx`: `(m_padded,)` int32, the ORIGINAL token index (row
      ...
      is what a later end-to-end combine/scatter step would need to route
      each expert's output back to its token.
  ...
  generate_local_shard_workload and the reference file's per-expert loop.
  """
  compute_dtype = hidden_states.dtype
...
def check_route_and_filter_correctness(
  ...
  route_and_filter_to_local_shard, per the user's explicit requirement
  (2026-08-26) to test this in isolation before wiring it into any
  end-to-end forward pass or benchmark. Pure JAX/numpy -- no tokamax
  dependency, runs anywhere (unlike the ragged_dot-based checks elsewhere
  in this file). `local_expert_start=137` (not 0) deliberately, so a bug
  that only breaks for a non-zero shard offset isn't masked.
  ...
  confirmed OOM-inducing on a single v6e chip per single_chip_kimi_k3_config's
  docstring, and not needed for a routing-only test anyway).
  ...
  for row in range(num_valid):
    ...
    # local expert id for this row -- recover via group boundaries in per_expert_counts
    # (equivalent info is already implicit in group_sizes/sort order; recomputed
    # independently here via the ORIGINAL topk arrays instead, for a truly separate check)
    row_global_id = None
    for slot in range(topk_idx_np.shape[1]):
      gid = int(topk_idx_np[tok, slot])
      if local_expert_start <= gid < local_expert_start + local_num_experts:
